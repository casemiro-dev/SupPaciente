...
// --------------------------------------------------------------------------
// MOTOR DO WIDGET DA SELEÇÃO BRASILEIRA (POLLING 30S)
// --------------------------------------------------------------------------
const FUTEBOL_CONFIG = {
  apiUrl: "https://www.thesportsdb.com/api/v1/json/3/latestsoccer.php", // URL Exemplo/Fallback
  pollingInterval: 30_000
};

let estadoPartidaAnterior = {
  idJogo: null,
  golsBrasil: 0,
  golsAdversario: 0,
  emAndamento: false
...
function inicializarWidgetFutebol() {
  // Executa imediatamente a primeira busca e ativa o loop de 30s
  buscarDadosPartida();
  setInterval(buscarDadosPartida, FUTEBOL_CONFIG.pollingInterval);
}

async function buscarDadosPartida() {
  try {
    const resposta = await fetch(FUTEBOL_CONFIG.apiUrl);
    if (!resposta.ok) return;
    const dados = await resposta.json();
    
    // Processa e procura se há algum jogo do Brasil ativo
    processarDadosFutebol(dados);
  } catch (erro) {
    console.error("[Widget Futebol] Falha ao buscar dados:", erro);
  }
}

function processarDadosFutebol(dados) {
  // Simulação de busca na estrutura da API. Ajuste conforme o JSON real retornado.
  const jogos = dados.events || [];
  const jogoBrasil = jogos.find(j => 
    (j.strHomeTeam === "Brazil" || j.strAwayTeam === "Brazil") && 
    (j.strStatus === "Live" || j.strStatus === "Not Started" || j.strStatus === "NS")
  );

  const widget = document.getElementById("widget-brasil");
  if (!widget) return;

  // REQUISITO AVANÇADO: Se não houver jogo do Brasil ativo, mantém o widget oculto
  if (!jogoBrasil) {
    widget.style.display = "none";
    estadoPartidaAnterior.emAndamento = false;
    estadoPartidaAnterior.idJogo = null;
    return;
  }

  // Identifica se é o mesmo jogo ou um novo
  const idJogoAtual = jogoBrasil.idEvent;
  const isHome = jogoBrasil.strHomeTeam === "Brazil";
  
  const golsBrasilAtual = parseInt(isHome ? jogoBrasil.intHomeScore : jogoBrasil.intAwayScore, 10) || 0;
  const golsAdversarioAtual = parseInt(isHome ? jogoBrasil.intAwayScore : jogoBrasil.intHomeScore, 10) || 0;

  // Atualiza os dados visuais no DOM
  const IsLive = jogoBrasil.strStatus === "Live";
  
  document.getElementById("fut-status").innerText = IsLive ? (jogoBrasil.strProgress || "Ao Vivo") : "Pré-Jogo";
  document.getElementById("fut-tempo").innerText = IsLive ? ((jogoBrasil.strTime || "0") + "'") : "Em breve";
  
  document.getElementById("escudo-home").src = jogoBrasil.strHomeBadge || "";
  document.getElementById("escudo-away").src = jogoBrasil.strAwayBadge || "";
  
  document.getElementById("placar-home").innerText = jogoBrasil.intHomeScore || "0";
  document.getElementById("placar-away").innerText = jogoBrasil.intAwayScore || "0";

  // Torna o widget visível se ele estava oculto
  if (widget.style.display === "none") {
    widget.style.display = "flex";
  }

  // Se o jogo acabou de iniciar no monitoramento, apenas salva o placar base atual para evitar triggers falsos
  if (!estadoPartidaAnterior.emAndamento || estadoPartidaAnterior.idJogo !== idJogoAtual) {
    estadoPartidaAnterior.emAndamento = true;
    estadoPartidaAnterior.idJogo = idJogoAtual;
    estadoPartidaAnterior.golsBrasil = golsBrasilAtual;
    estadoPartidaAnterior.golsAdversario = golsAdversarioAtual;
    return;
  }

  // COMPARAÇÃO DE PLACAR: Valida se houve mudança desde o último polling
  if (golsBrasilAtual > estadoPartidaAnterior.golsBrasil) {
    comemorarGolBrasil();
  } else if (golsAdversarioAtual > estadoPartidaAnterior.golsAdversario) {
    sofrerGolAdversario();
  }

  // Atualiza o estado de referência para os próximos 30 segundos
  estadoPartidaAnterior.golsBrasil = golsBrasilAtual;
  estadoPartidaAnterior.golsAdversario = golsAdversarioAtual;
}

function comemorarGolBrasil() {
  // 1. Lança o Toast customizado do sistema
  if (typeof window.lancarToast === "function") {
    window.lancarToast("⚽ GOOOOOL DO BRASIL!", "success");
  }

  // 2. Dispara a chuva de confetes utilizando canvas-confetti
  if (typeof confetti === "function") {
    confetti({ particleCount: 150, spread: 80, origin: { y: 0.6 } });
  }
}

function sofrerGolAdversario() {
  // 1. Lança o Toast de frustração
  if (typeof window.lancarToast === "function") {
    window.lancarToast("😢 Gol do adversário...", "danger");
  }

  // 2. Altera temporariamente as cores do componente adicionando a classe CSS
  const widget = document.getElementById("widget-brasil");
  if (widget) {
    widget.classList.add("sofreu-gol");
    setTimeout(() => {
      widget.classList.remove("sofreu-gol");
    }, 5000); // Remove o estado de alerta e tremor após 5 segundos
  }
}

// Vincula a inicialização do módulo ao gatilho de carregamento do DOM que você já possui
window.addEventListener("DOMContentLoaded", () => {
  inicializarWidgetFutebol();
});